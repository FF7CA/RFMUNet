...
class MaskedDiagonalConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, padding=1, diagonal_type='main', groups=1):
        super(MaskedDiagonalConv2d, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, padding=padding, bias=False, groups=groups)
        
        mask = torch.zeros(kernel_size, kernel_size)
        if diagonal_type == 'main':
            mask.fill_diagonal_(1)
        else:
            mask.fliplr().fill_diagonal_(1)
            
        self.register_buffer('mask', mask.view(1, 1, kernel_size, kernel_size))

    def forward(self, x):
        masked_weight = self.conv.weight * self.mask
        return nn.functional.conv2d(x, masked_weight, bias=None, stride=self.conv.stride, 
                                    padding=self.conv.padding, dilation=self.conv.dilation,
                                    groups=self.conv.groups)

class AnisotropicDirectionalAttention(nn.Module):
    def __init__(self, channel, reduction=4, kernel_size=3):
        super(AnisotropicDirectionalAttention, self).__init__()
        padding = kernel_size // 2
        d_channel = channel // reduction

        self.reduce = nn.Sequential(
            nn.Conv2d(channel, d_channel, 1, bias=False),
            nn.BatchNorm2d(d_channel),
            nn.ReLU(inplace=True)
        )
        
        self.conv_h = nn.Conv2d(d_channel, d_channel, (1, kernel_size), padding=(0, padding), bias=False, groups=d_channel)
        self.conv_v = nn.Conv2d(d_channel, d_channel, (kernel_size, 1), padding=(padding, 0), bias=False, groups=d_channel)
        self.conv_d1 = MaskedDiagonalConv2d(d_channel, d_channel, kernel_size, padding=padding, diagonal_type='main', groups=d_channel)
        self.conv_d2 = MaskedDiagonalConv2d(d_channel, d_channel, kernel_size, padding=padding, diagonal_type='anti', groups=d_channel)

        self.bn_fusion = nn.BatchNorm2d(channel)

        self._initialize_weights()

    def _initialize_weights(self):
        nn.init.constant_(self.bn_fusion.weight, 0)
    ...
    def forward(self, x):
        ...
        h_out = self.conv_h(x_reduced)
        v_out = self.conv_v(x_reduced)
        d1_out = self.conv_d1(x_reduced)
        d2_out = self.conv_d2(x_reduced)

        concat_out = torch.cat([h_out, v_out, d1_out, d2_out], dim=1)

        concat_out = concat_out.view(B, 4, C // 4, H, W).transpose(1, 2).contiguous().view(B, C, H, W)

        A_dir = torch.sigmoid(self.bn_fusion(concat_out))

        return ((x * A_dir) + x).permute(0, 2, 3, 1).contiguous() # to (B, H, W, C)
...
class SS2Dv0:
    def __initv0__(
        ...
        **kwargs,
        ...
        self.ada = AnisotropicDirectionalAttention(channel=d_inner)

    def forwardv0(self, x: torch.Tensor, seq=False, force_fp32=True, **kwargs):
        x = self.in_proj(x)
        x, z = x.chunk(2, dim=-1) # (b, h, w, d)
        z = self.act(self.ada(z))
        x = x.permute(0, 3, 1, 2).contiguous()
